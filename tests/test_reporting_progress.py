...
def test_washtenaw_progress_distinguishes_exists_from_inferred_reported_without_completion():
    ...
    assert rows[0].basis == ReportingProgressBasis.SOURCE_REPORTED
    ...
    assert all(r.update_semantics == UpdateSemantics.CUMULATIVE for r in rows)
...
def test_progress_transition_history_is_monotonic_when_source_counts_are_cumulative():
    import json
    def parse(n):
        payload={"status":{"precinctsReporting":n,"precinctsTotal":10,"cumulative":"cumulative"}}
