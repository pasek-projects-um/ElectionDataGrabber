...
STATE_COUNTS={"HI":4,"MO":116,"NH":234,"NJ":21}
...
def promotable_rows(expansion_path: Path) -> list[dict[str,str]]:
    ...
    for r in rows:
        ...
        if not url or not host: continue
        key=(r["state"],url)
        if key in seen: continue
        seen.add(key)
        out.append({
            "state":r["state"],"authority_url":url,"authority_host":host,
...
def summarize(rows: list[dict[str,str]]) -> list[dict[str,str]]:
    ...
    for state in sorted(STATE_COUNTS):
        ...
        out.append({
            "state":state,"expected_primary_units":str(STATE_COUNTS[state]),
...
if __name__=="__main__":
    ...
    write(rows,root/"audit/mo_nh_hi_nj_promotable_surfaces.csv")
    write(summarize(rows),root/"audit/mo_nh_hi_nj_promotion_summary.csv")
