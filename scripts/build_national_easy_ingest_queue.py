...
def main() -> None:
    ...
    denominators = {
        d.state: d.expected_primary_units
        for d in read_denominators(root / "registry/us_primary_election_locality_denominators.csv")
    }
    candidates = build_candidates(expansion)
    write_rows(root / "audit/national_easy_ingest_candidates.csv", candidates, FIELDS)
    write_rows(root / "audit/national_easy_ingest_summary.csv", summarize(candidates, denominators), SUMMARY_FIELDS)
    write_rows(
        root / "audit/national_supported_ingest_manifest.csv",
        supported_ingest_manifest(candidates),
        MANIFEST_FIELDS,
    )
